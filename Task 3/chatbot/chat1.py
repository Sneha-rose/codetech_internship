...
try:
    ...
    if 'intents' not in data or not isinstance(data['intents'], list):
        print("❌ Error: Invalid structure in intents.json")
        exit(1)
except (FileNotFoundError, json.JSONDecodeError):
    print("❌ Error: Could not load intents.json")
    exit(1)
...
for intent in data['intents']:
    if isinstance(intent, dict) and 'tag' in intent and 'patterns' in intent:
        ...
        print("⚠️ Warning: Skipping malformed intent entry in JSON")
...
if intent_patterns:
    ...
    print("❌ Error: No training patterns found in intents.json")
    exit(1)
